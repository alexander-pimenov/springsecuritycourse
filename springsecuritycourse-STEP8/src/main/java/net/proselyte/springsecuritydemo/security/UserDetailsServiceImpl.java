 ...
 * Использует {@link UserRepository} для поиска по email и оборачивает
 * JPA-сущность {@link net.proselyte.springsecuritydemo.model.User}
 * в {@link SecurityUser}, совместимый со Spring Security.
 */
...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        User user = userRepository.findByEmail(email).orElseThrow(() ->
                new UsernameNotFoundException("User with email='%s' doesn't exists".formatted(email)));
